...
-- Ensure policy exists (drop then create for idempotency)
drop policy if exists "Categories are viewable by everyone" on public.tcgcsv_categories;
...
-- Recreate trigger (drop if exists for idempotency)
drop trigger if exists trg_tcgcsv_categories_updated_at on public.tcgcsv_categories;
...
drop policy if exists "Sync logs are viewable by everyone" on public.sync_logs;
